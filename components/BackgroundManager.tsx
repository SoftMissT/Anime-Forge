...
const createParticles = (count: number, className: string, content?: string[]) => {
    return Array.from({ length: count }).map((_, i) => {
        ...
        if (className === 'ember') {
            ...
            style['--wind-direction'] = String(Math.random() - 0.5);
...
const BackgroundManagerComponent: React.FC<{ view: AppView }> = ({ view }) => {
...
export const BackgroundManager = memo(BackgroundManagerComponent);
